  ...
  return (
    <div className="flex w-full min-h-screen justify-center items-center">
      <div className="w-full sm:w-1/2">
        <h2 className="text-3xl text-center text-slate-100">{loading ? "processing" : "SignUp"}</h2>
        <form className="mt-10" action="">
          ...
          <div className="mt-10 px-3 space-y-5">
            ...
            <button disabled={loginBtnDisable} onClick={onSubmit} type="submit" className="w-full bg-slate-200 hover:scale-95 active:bg-slate-300 rounded-full px-5 py-1 text-black font-semibold">{ loginBtnDisable ? "Fill Please" : "SignUp" }</button>
            <p className="text-sm text-center text-gray-400">Alredy has Account? <Link className="text-gray-200" href="/login">Login here</Link></p>
          </div>
        </form>
      </div>
    </div>
